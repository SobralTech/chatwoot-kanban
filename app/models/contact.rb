...
#  index_contacts_on_email_trgm                          (lower((email)::text) gin_trgm_ops) USING gin
#  index_contacts_on_lower_email_account_id              (lower((email)::text), account_id)
#  index_contacts_on_name_email_phone_number_identifier  (name,email,phone_number,identifier) USING gin
#  index_contacts_on_name_trgm                           (immutable_unaccent(lower((name)::text)) gin_trgm_ops) USING gin
#  index_contacts_on_nonempty_fields                     (account_id,email,phone_number,identifier) WHERE (((email)::text <> ''::text) OR ((phone_number)::text <> ''::text) OR ((identifier)::text <> ''::text))
#  index_contacts_on_phone_digits_trgm                   (regexp_replace((phone_number)::text, '\D'::text, ''::text, 'g'::text) gin_trgm_ops) USING gin
