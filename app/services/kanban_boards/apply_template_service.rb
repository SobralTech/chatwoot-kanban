module KanbanBoards
  class ApplyTemplateService
    def initialize(kanban_board:, template_key: nil)
      ...
      @template_key = TemplateCatalog.key_for(template_key)
      @template = TemplateCatalog.fetch(@template_key)
    end
    ...
    def perform!
      KanbanBoard.transaction do
        I18n.with_locale(account.locale.presence || I18n.default_locale) do
          ...
          won_stage = create_terminal_stage(@template[:won], regular_stages.length + 1, TemplateCatalog::WON_COLOR)
          lost_stage = create_terminal_stage(@template[:lost], regular_stages.length + 2, TemplateCatalog::LOST_COLOR)
